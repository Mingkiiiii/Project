<?php
  ...
  $cooktip_data = dbFetch("SELECT *
  FROM cooktip 
  where idx = ?", [
      $idx,
  ]);

  if (!$cooktip_data) msgMove('접근할 수 없는 페이지입니다.', '/');

  if ($cooktip_data['writer_id'] != $mb_r['user_id']) {
      ...
      <div id="section">
        <section id="my-recipe">
          ...
          <div class="recipe-write-form">
            <div class="container">
              <form method="post" enctype="multipart/form-data" onSubmit="return frmSubmit(this, '/ajax/cooktip/cooktip-update.php', '요리팁을 수정했습니다.', './cooktip.php');">
                <input type="hidden" name="cooktip_idx" value="<?= $_GET['idx'] ?>" >
                <div class="form-group">
                  <label for="title">제목</label>
                  <input type="text" name="title" id="title" class="form-control" placeholder="제목을 입력해주세요." value="<?= $cooktip_data['title'] ?>" required>
                </div>

                <div class="form-group">
                  <label for="content">레시피 내용</label>
                  <textarea type="text" name="content" id="content" class="form-control" style="height:500px; width:100%;" placeholder=''><?= $cooktip_data['content'] ?></textarea>
                </div>
