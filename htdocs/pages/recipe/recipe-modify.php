...
// 레시피 데이터
$recipe_data = dbFetch("SELECT r.*, m.user_name 
...
if ($recipe_data['writer_id'] != $mb_r['user_id']) {
    msgMove('작성자만 수정 가능합니다.', '/');
}
?>
            ...
            <div id="section">
                <section id="my-recipe">
                    ...
                    <div class="recipe-write-form">
                        <div class="container">
                            <form method="post" enctype="multipart/form-data" onSubmit="return frmSubmit(this, '/ajax/recipe/recipe-modify.php', '레시피를 수정했습니다.', './recipe-view.php?idx=<?= $idx ?>');">
                                <input type="hidden" name="recipe_idx" value="<?= $idx ?>">

                                <div class="form-group">
                                    <label for="recipe_name">레시피 이름</label>
                                    <input type="text" name="recipe_name" id="recipe_name" class="form-control" placeholder="레시피 이름을 입력해주세요." value="<?= $recipe_data['recipe_name'] ?>" required>
                                </div>

                                <div class="form-group">
                                    <label for="recipe_youtube">유튜브 영상</label>
                                    <textarea type="text" name="youtube_video" id="youtube_video" class="form-control" placeholder='YOUTUBE 동영상 ID(ex: lKyOIDYn5Xo)'><?= $recipe_data['recipe_youtube'] ?></textarea>
                                </div>

                                <div class="form-group">
                                    <label for="recipe_thumbnail">레시피 썸네일</label>
                                    <div class="thumbnail-image">
                                        <img src="/uploads/recipe/<?= $recipe_data['recipe_youtube'] ?>" alt="레시피 썸네일" id="thumbnail-image" style="width:200px;">
                                    </div>
                                    ...
                                    <div class="thumbnail-file">
                                        ...
                                        <input type="hidden" name="recipe_image_name" id="recipe_image_name" value="<?= $recipe_data['recipe_image'] ?>" />
                                    </div>
                                </div>

                                <div class="form-group">
                                    <label for="recipe_text">레시피 내용</label>
                                    <textarea type="text" name="recipe_text" id="recipe_text" class="form-control" style="height:500px; width:100%;" placeholder=''><?= $recipe_data['recipe_text'] ?></textarea>
                                </div>
